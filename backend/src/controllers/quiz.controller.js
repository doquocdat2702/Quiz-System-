...
async function getQuizByCode(req, res, next) {
  try {
    ...
    if (!quiz) {
      return res.status(404).json({ success: false, message: "Không tìm thấy bài test với mã này" });
    }
...
async function getQuizForEdit(req, res, next) {
  try {
    ...
    if (String(quiz.created_by) !== String(req.user.id)) {
      return res.status(403).json({ success: false, message: "Bạn không có quyền sửa bài test này" });
    }
...
async function createQuiz(req, res, next) {
  ...
  try {
    ...
    while (attempts < 10) {
      code = generateCode();
      const existing = await pool.query("SELECT id FROM quizzes WHERE code = $1", [code]);
    ...
    console.log(`[QUIZ] User ${req.user.email} created quiz: "${title}" (id: ${newQuiz.id}, code: ${code})`);
...
async function updateQuiz(req, res, next) {
  ...
  try {
    ...
    if (!quizResult.rows[0]) {
      ...
      if (!exists.rows[0]) return res.status(404).json({ success: false, message: "Quiz không tồn tại" });
      return res.status(403).json({ success: false, message: "Bạn không có quyền sửa bài test này" });
    }
    ...
    res.json({ success: true, message: "Cập nhật quiz thành công", data: { quiz: quizResult.rows[0] } });
  } catch (err) {
    if (client) await client.query("ROLLBACK");
    next(err);
  } finally {
...
async function deleteQuiz(req, res, next) {
  try {
    ...
    if (!result.rows[0]) {
      const exists = await pool.query("SELECT id FROM quizzes WHERE id = $1", [req.params.id]);
      if (!exists.rows[0]) return res.status(404).json({ success: false, message: "Quiz không tồn tại" });
      return res.status(403).json({ success: false, message: "Bạn không có quyền xóa bài test này" });
    }
