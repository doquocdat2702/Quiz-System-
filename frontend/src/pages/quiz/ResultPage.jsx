  ...
  useEffect(() => {
    ...
    const fetchAttempt = async () => {
      try {
        ...
        toast.error(error.message || "Không tải được kết quả");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50">
        <Navbar />
        <div className="p-10 text-center text-xl text-gray-600">Đang tải kết quả...</div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50">
        ...
        <div className="p-10 text-center">
          <p className="text-xl text-gray-600 mb-6">Không có dữ liệu kết quả</p>
      ...
      <main className="px-5 md:px-12 py-8">
        <div className="max-w-4xl mx-auto">
          <section className="bg-gradient-to-r from-green-600 to-emerald-500 text-white rounded-2xl p-6 mb-6 shadow-lg">
            <p className="text-green-50 mb-2">{attempt.quiz || attempt.quizTitle || "Kết quả bài test"}</p>
            <h1 className="text-4xl md:text-5xl font-black mb-5">Kết quả</h1>
            <div className="flex flex-col md:flex-row md:items-end gap-4 md:gap-8">
              <p className="text-6xl font-black">
                {attempt.score}/{attempt.total}
              </p>
              <p className="text-2xl font-semibold text-green-50">{percent}% chính xác</p>
            </div>
          </section>
                    ...
                    {(item.answers || []).map((answer, answerIndex) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {answer}
                          {isCorrect && <span className="ml-2 text-green-700 font-semibold">Đáp án đúng</span>}
