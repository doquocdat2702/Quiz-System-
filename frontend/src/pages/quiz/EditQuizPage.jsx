...
function EditQuizPage() {
  ...
  useEffect(() => {
    const loadQuiz = async () => {
      try {
        ...
        toast.error(error.message || "Khong tai duoc bai test de sua");
        navigate("/");
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      toast.success("Da cap nhat bai test");
      navigate("/");
    } catch (error) {
      toast.error(error.message || "Khong cap nhat duoc bai test");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50">
        <Navbar />
        <div className="p-10 text-center text-xl text-gray-600">Dang tai...</div>
      </div>
    );
  }
      ...
      <main className="px-5 md:px-12 py-8">
        <div className="max-w-5xl mx-auto">
          <div className="mb-8 rounded-2xl bg-gradient-to-r from-indigo-600 to-blue-600 p-6 md:p-8 text-white shadow-lg">
            <h1 className="text-3xl md:text-5xl font-black">Sua bai test</h1>
            <p className="text-indigo-50 mt-3 text-lg">
              Cap nhat cau hoi, dap an va dap an dung cua bai test.
            </p>
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {item.answers.map((answer, answerIndex) => (
                    <label key={answerIndex} className="flex gap-3 items-center border border-gray-200 rounded-lg px-4 py-3">
                      ...
                      />
                      <span className="text-xs font-semibold text-gray-500 whitespace-nowrap">Dung</span>
                    </label>
                  ))}
                </div>
              </section>
