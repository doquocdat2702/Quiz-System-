...
function QuizPage() {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      try {
        ...
        toast.error("Khong tai duoc bai test");
        console.error(error);
        navigate("/");
      } finally {
        setLoading(false);
  ...
  const handleSubmit = async () => {
    ...
    if (answeredCount !== questions.length) {
      toast.error("Vui long tra loi tat ca cau hoi");
      return;
    }
    ...
    try {
      ...
      toast.error(error.message || "Loi khi nop bai");
      console.error(error);
    } finally {
      setSubmitting(false);
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="text-xl text-gray-600">Dang tai...</div>
      </div>
    );
  }
  ...
  if (!quiz) {
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="text-xl text-gray-600">Khong tim thay bai test</div>
      </div>
    );
  }
        ...
        <div className="max-w-4xl mx-auto">
          <div className="mb-8 rounded-2xl bg-white border border-gray-200 p-6 shadow-sm">
            <div className="flex flex-wrap gap-3 items-center mb-3">
              ...
              <span className="text-gray-500">{questions.length} cau hoi</span>
            </div>
          ...
          <div className="sticky bottom-0 bg-slate-50/95 py-4 border-t border-gray-200 backdrop-blur">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <p className="text-gray-600">
                Da tra loi {Object.keys(userAnswers).length}/{questions.length} cau
              </p>
              <button
                type="button"
