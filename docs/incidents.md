...
## Incident 1: Backend khong ket noi duoc MySQL

- Hien tuong: Backend container thoat hoac API tra loi loi 500 khi goi cac endpoint can database.
- Layer loi: L2 - Database.
- Nguyen nhan: `DB_HOST`, `DB_PASSWORD`, `DB_NAME` sai hoac MySQL chua san sang khi backend khoi dong.
- Cach fix: Dung `DB_HOST=db` trong Docker Compose, kiem tra `docker compose logs db`, sua bien moi truong trong `.env`.
- Cach phong tranh: Luu cau hinh mau trong `.env.example`, dung healthcheck MySQL va de backend phu thuoc `service_healthy`.
...
- Cach fix: Stop process dang giu port hoac doi `BACKEND_PORT`, `FRONTEND_PORT`, `MYSQL_PORT` trong `.env`.
